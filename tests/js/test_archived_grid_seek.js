...
async function run() {
...

    {
        ...
        const dtPlusDom = createDomTable(250);
        const seek = loadSeek({ jQuery: env.jq, $: env.jq, ARCHIVED_GRID_SEEK_TIMEOUT_MS: 20 });
        const result = await seek(dtPlusDom, 200, 200);
        assert.strictEqual(result.start_row, 0, "failed API seek must keep start_row at 0 even if a deep <tr> exists");
        assert.notStrictEqual(result.seek_mode, "datatable");
        assert.notStrictEqual(result.seek_mode, "dom_absolute", "do not fall through to DOM when DataTables is controlling the grid");
    }
