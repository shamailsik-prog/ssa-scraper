...
| 0.5, 3.2, 3.7 | Saved per-slot username/password (encrypted at rest, migration 006); a `recover-login-slots` Beat task re-verified a bounced slot after 15 minutes and, if dead, signed in again with the saved credentials; a paused source resumed on its own | Removed by #123 (migration 007 dropped the columns). **Restored the same day at the operator's instruction** (after the human login had succeeded: "the passwords were saved and used to log in automatically; tick the box below the password"): migration 008 re-adds the columns, the credential endpoints and the Human login tab's saved sign-in form are back, the recovery task runs every 5 minutes on the single login-session worker (never beside a harvest job), and the autofill ticks every unticked box in the sign-in form before pressing Sign in. This departs from sections 0.5 and 3.2 of the document at the operator's decision; verification pages are still never solved. |
...
| `tasks/spot_check.py`, migration 009 | Every 30 minutes three promoted PakistanLawSite judgments (through the login session, under the ordinary pacing and lock, on the login-session worker) and two current statute sections (opened in a headless browser under the public fetcher's allow-list, robots policy and pacing; PDFs fetched by the browser) are re-read and compared with what the corpus holds, on the page as the browser shows it rather than on a bare download (operator's instruction of 24 September); each check is a `spot_check` row and `/status` shows the last 24 hours and the latest 20 | Requested by the operator on 24 September so the corpus can be seen to be right, not only large. Nothing in the corpus is changed by a check. |
| `storage/adapters.py` (Google Drive), `cloud/do_server_logs.sh` | The Drive adapter takes an OAuth grant from the account that owns the folder (a personal Drive; a service account has no storage of its own) or a service account for a Workspace Shared Drive, and passes `supportsAllDrives` on every call; the logs workflow's `status` word prints a read-only harvest report from the database (counts, speed per hour, sources, slots, archive mirrors and backlog) and `summary` prints `/status.json`; a refused sign-in records the site's own answer (invalid credentials, account already in use, account inactive, terms not accepted) | Ported from #124 (the other session's line, 24 September); the two-login and harvest-mode parts of #124 were not taken because the operator asked for one login at a time. |
| `routers/archive.py` (Connect Google Drive, existing folder) | The connect form takes an optional link of a folder that already exists in that Drive; the consent then asks for the full Drive permission (an existing folder is invisible under `drive.file`), the callback verifies the folder is reachable and stores its id instead of creating "SIKANDER AI Corpus" | Requested by the operator on 24 September so the copies continue in the folder an earlier service filled (SIKANDER_AI_Corpus_Archive), under one service. |
