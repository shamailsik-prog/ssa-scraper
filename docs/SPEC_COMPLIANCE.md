...
| `tasks/spot_check.py`, migration 009 | Every 30 minutes three promoted PakistanLawSite judgments (through the login session, under the ordinary pacing and lock, on the login-session worker) and two current statute sections (public fetcher) are re-fetched and compared with what the corpus holds; each check is a `spot_check` row and `/status` shows the last 24 hours and the latest 20 | Requested by the operator on 24 September so the corpus can be seen to be right, not only large. Nothing in the corpus is changed by a check. |
